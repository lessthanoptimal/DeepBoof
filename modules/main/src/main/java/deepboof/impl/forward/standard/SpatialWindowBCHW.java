 ...
 * @author Peter Abeles
 */
public abstract class SpatialWindowBCHW
		...
		extends BaseSpatialWindow<T,VT>
{
	...
	public SpatialWindowBCHW(ConfigSpatial config, VT padding) {
		...
		if(isEntirelyBorder(outR0, outC0)) {
			...
			for (int batchIndex = 0; batchIndex < N; batchIndex++) {
				for( int channel = 0; channel < C; channel++ ) {
					forwardsBorder(batchIndex,channel, 0, 0, Ho, Wo);
				}
			}
		} else {
			...
			for (int batchIndex = 0; batchIndex < N; batchIndex++) {
				for( int channel = 0; channel < C; channel++ ) {
					...
					forwardsBorder(batchIndex,channel, 0, 0, outR0, Wo);
					forwardsBorder(batchIndex,channel, outR1, 0, Ho, Wo);
					forwardsBorder(batchIndex,channel, outR0, 0, outR1, outC0);
					forwardsBorder(batchIndex,channel, outR0, outC1, outR1, Wo);
	 ...
	 */
	private void forwardsBorder(int batchIndex , int channel , int row0, int col0, int row1, int col1 ) {
