...
public abstract class SpatialWindowBHWC
		<T extends Tensor<T>,VT extends BaseSpatialPadding2D<T>>
		extends BaseSpatialWindow<T,VT> {

	// reference to output tensor
	protected T output;

	public SpatialWindowBHWC(ConfigSpatial config, VT padding) {
	 ...
	 * @param outX x-axis output coordinates
	 */
	protected abstract void forwardAt_border(VT padded, int batch, int padY, int padX, int outY, int outX);
