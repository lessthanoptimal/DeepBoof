...
public class TestSpatialWindowBCHW extends ChecksSpatialWindow {
	...
	public class Helper extends SpatialWindowBCHW<Tensor_F64,ConstantPadding2D_F64> {
		...
		protected void forwardsAt(Tensor_F64 input, int batch, int channel, int inY, int inX, int outY, int outX) {
			...
			output.d[output.idx(batch,channel,outY,outX)] = sum;
		}

		@Override
		protected void forwardsAt(ConstantPadding2D_F64 padded, int batch, int channel, int padY, int padX, int outY, int outX) {
			double sum = 0;
			for (int y = 0; y < HH; y++) {
				for (int x = 0; x < WW; x++) {
